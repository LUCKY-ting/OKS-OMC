%Our accelerated OKS-OMC. It uses 13 Gaussian kernels of the form k(x,y)=exp(-||x-y||^2/2*scale)
% where scale takes in the set {2.^{-6:1:6}}.
...
eta = 2.^(-1);
beta = eta/maxIterNum;
scale = 2.^(-6:1:6); %kernel hyperparameter
...
for run = 1:times
    ...
    for i = 1:n
        ...
        if i > 1
            if strcmp(stage,'first') %make a combined prediction
                for p = 1:P
                    if i<= initNum
                ...
                if i<= initNum
                    km{opt} = kernelMatrix(SVsIdx(1,1:SVsNum(opt),opt),i,opt);
                else
                    SVsMatrix = data(SVsIdx(1,1:SVsNum(opt),opt),:);
                    km{opt} = rbfkernel(SVsMatrix',x,scale(opt));
                end
                pred_v(:,:,opt) = km{opt}'* coff(1:SVsNum(opt),:,opt);
                pred_val =  pred_v(:,:,opt);
            end
        end
        ...
        if strcmp(stage,'first')   %perform online kernel selection and online model updating
            ...
            if norm(prev_miu - miu, 1) < 1e-5
                constantTimes = constantTimes + 1;
                if constantTimes == 100
                    stage = 'second'; disp(['step into the second stage at Round ' num2str(i)]);
                    [~,opt] = max(miu);
                end
            else
                constantTimes = 0;
            ...
            for iter = 1:maxIterNum
                coff(:,:,opt) = (1 - lambda*eta) * coff(:,:,opt);
                a_t = 0;
                b_t = 0;
                cur_coeff = zeros(1, L+1);
                for k = 1:L
                    if y(k) == 1 &&  (pred_v(1,k,opt) -  pred_v(1,L+1,opt) < 1)
                        a_t = a_t + 1;
                        cur_coeff(k) = eta / Y_t_size;
                        cur_coeff(L+1) = cur_coeff(L+1) - eta/Y_t_size;
                        pred_v(1,k,opt) = (1 - lambda*eta) * pred_v(1,k,opt) + cur_coeff(k); %re-compute the predicted value
                    elseif y(k) == 0 &&  (pred_v(1,L+1,opt) -  pred_v(1,k,opt) < 1)
                        b_t = b_t + 1;
                        cur_coeff(k) = - eta / (L - Y_t_size);
                        cur_coeff(L+1) = cur_coeff(L+1) + eta/(L - Y_t_size);
                        pred_v(1,k,opt) = (1 - lambda*eta) * pred_v(1,k,opt) + cur_coeff(k); %re-compute the predicted value
                    else
                        pred_v(1,k,opt) = (1 - lambda*eta) * pred_v(1,k,opt);
                    end
                end
                
                pred_v(1,L+1,opt) = (1 - lambda*eta) * pred_v(1,L+1,opt) + cur_coeff(L+1);
                ...
                if iter == 1   %store SVs and their coefficients
                    SVsNum(opt) = SVsNum(opt) + 1;
                    if i<= initNum
                        SVsIdx(1,SVsNum(opt),opt) = i;
                    else
                        SVsIdx(1,SVsNum(opt),opt) = j;
                    end
                    curId = SVsNum(opt);
                    km{opt} = [km{opt}; 1];
                end
                coff(curId,:,opt) = coff(curId, :, opt) + cur_coeff;
            end
        end
...
fid = fopen(['OKS-OMC-GK-' dname '.csv'],'a');
...
save(['OKS-OMC-GK-metrics-' dname '.mat'],'metrics');
save(['OKS-OMC-GK-outputmiu-' dname '.mat'],'output_miu');
