% Our accelerated OKS-OMC using mS-3 isolation kernels 
% where psi takes values in the set {2.^{4:1:mS}}
...
%     save(fileName,'mappingData','labels');
    [v, a, mu, wsNorm, metrics(run,:)] = OKS_OMC_diffStep_checkConvergence(mappingData',dataLabels', index, epoch, t, P, lambda, eta, beta, maxIter);
    output_miu(run,:) = mu;
end
...
fid = fopen(['OKS-OMC-IK-' dname '.csv'],'a');
fprintf(fid,'runTimes, epoch, maxIter, P, lambda, eta, beta, precision, +std, recall, +std, F1score, +std, macro_F1score, +std, micro_F1score, +std, hammingloss, +std, subsetAccuracy, +std, rankingLoss, +std, oneErr, +std, time [s] \n ');
fprintf(fid, '%d, %d, %d, %d, %g, %g, %g, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f \n', ...
    times, epoch, maxIter, P, lambda, eta, beta, mean(metrics(:,1)), std(metrics(:,1)), mean(metrics(:,2)), std(metrics(:,2)), mean(metrics(:,3)), std(metrics(:,3)),...
...
fclose(fid);
save(['OKS-OMC-IK-metrics-' dname '.mat'],'metrics');
save(['OKS-OMC-IK-outputmiu-' dname '.mat'],'output_miu');
